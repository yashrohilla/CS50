...
main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        //printf("g.top = %d", g.top);
        //printf("g.left = %d", g.left);
...
move_cursor(void)
{
  ...
  int ch = 0; // initialize the cursor variable

    while(ch != ' ')
    {
      ch = getch();
    if(ch == 259)   // User input: Up arrow key
    {
      g.y -= 1;
      show_cursor();
    }
    else if(ch == 260)  // User input: Left arrow key
    {
      g.x -= 1;
      show_cursor();
    }
    else if(ch == 261) // User input: Right arrow key
    {
      g.x += 1;
      show_cursor();
    }
    else if(ch == 258) // User input: Down arrow key
    {
      g.y += 1;
      show_cursor();
    }
    ...
    if(g.x == 9) // If cursor hits right wall, then stop
    {
      g.x = 8;
      show_cursor();
    }
    if(g.y == -1) // If cursor hits top, then stop
    {
      g.y = 0;
      show_cursor();
    }
    if(g.y == 9) // If cursor hits bottom, then stop
    {
      g.y = 8;
      show_cursor();
    }
